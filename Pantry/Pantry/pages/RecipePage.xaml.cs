...
namespace Pantry.pages
{
    ...
    public partial class RecipePage : ContentPage
    {
        ...
        private int selectedCalories = 0;
        ...
        public RecipePage()
        {
            ...
            TappedItem = new Command((object s) =>
            {
                ...
                Navigation.ShowPopup(new RecipeInfoPage(recipe.Title, ingredients, recipe.Description, recipe.ImageSource, recipe.calories));
            });
        ...
        private async void BtnRecipeFilter(object sender, EventArgs e)
        {
            try
            {
                List<object> items = (List<object>)await Navigation.ShowPopupAsync(new RecipeFilterPage(selectedType, startDate, endDate, selectedCalories, ingredients));
                selectedType = (string)items[0];
                startDate = (DateTime?)items[1];
                endDate = (DateTime?)items[2];
                selectedCalories = (int)items[3];
                ingredients = (List<string>)items[4];
                Update(this, null);
            }
        ...
        public void Update(object sender, EventArgs args)
        {
            ...
            {
                ordered = from recipe in ordered
                                                     where recipe.Type == selectedType
                                                     select recipe;
            ...
            {
                ordered = from recipe in ordered
                                                     where recipe.calories <= selectedCalories
                                                     select recipe;
            ...
            if (ingredients.Count != 0)
            {
                ...
                {
                    ordered = ordered.Where(list =>
                    {
                        foreach (var ingredient1 in list.Ingredients)
                        {
                            if (ingredient1.Name == ingredient)
                            {
                                return true;
                            }
                        }
